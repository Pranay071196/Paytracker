...
import { fetchCollectionsForOrganiser, fetchParticipantsForCollection, updateCollectionWithParticipants, updateProfileUpiId } from '../lib/supabaseHelpers'
import { fetchGroupsForOrganiser, fetchGroupMembers } from '../lib/supabaseGroupHelpers'
import { normalizePhoneNumber } from '../auth/LoginScreen'
...
export default function EditCollection() {
  ...
  useEffect(() => {
    if (!profile?.id || !collectionId) return

    Promise.all([
      fetchCollectionsForOrganiser(profile.id),
      fetchParticipantsForCollection(collectionId),
      fetchGroupsForOrganiser(profile.id),
    ])
      .then(([collections, participantData, groupsData]) => {
        ...
        setAmount(String(collection.target_amount))
        setParticipants(participantData.map(p => p.participant_phone))
        setGroups(groupsData)
      })
      .catch(() => navigate('/organiser-dashboard'))
      .finally(() => setLoading(false))
  }, [profile, collectionId, navigate])
  ...
  const handleUpdateCollection = async () => {
    ...
    if (profile && profile.id) {
      try {
        ...
        await updateCollectionWithParticipants(collectionId, profile.id, {
          title,
          category,
  ...
  if (loading) {
    return (
      <main className="modal-screen">
        <div className="modal-card" style={{ padding: 40, textAlign: 'center', color: '#94a3b8' }}>
          Loading...
        </div>
      </main>
    )
